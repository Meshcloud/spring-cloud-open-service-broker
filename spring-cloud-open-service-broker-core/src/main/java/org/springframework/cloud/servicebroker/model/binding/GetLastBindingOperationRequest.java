/*
 * Copyright 2002-2018 the original author or authors.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *       http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.springframework.cloud.servicebroker.model.binding;

import org.springframework.cloud.servicebroker.model.Context;
import org.springframework.cloud.servicebroker.model.ServiceBrokerRequest;
import org.springframework.cloud.servicebroker.service.ServiceInstanceBindingService;

import java.util.Objects;

/**
 * Details of a request to get the state of the last operation on a service instance.
 *
 * <p>
 * Objects of this type are constructed by the framework from the headers, path variables, query parameters
 * and message body passed to the service broker by the platform.
 *
 * @author Scott Frederick
 * @see <a href="https://github.com/openservicebrokerapi/servicebroker/blob/master/spec.md#request-1">Open Service Broker API specification</a>
 */
public class GetLastBindingOperationRequest extends ServiceBrokerRequest {
    private transient String serviceInstanceId;

    private transient String serviceBindingId;

    protected transient String operation;

    GetLastBindingOperationRequest(String serviceInstanceId, String serviceBindingId,
                                   String operation,
                                   String platformInstanceId, String apiInfoLocation,
                                   Context originatingIdentity) {
        super(platformInstanceId, apiInfoLocation, originatingIdentity);
        this.serviceInstanceId = serviceInstanceId;
        this.serviceBindingId = serviceBindingId;
        this.operation = operation;
    }

    /**
     * Get the ID of the service instance.
     *
     * <p>
     * This value is set from the {@literal :instance_id} path element of the request from the platform.
     *
     * @return the service instance ID
     */
    public String getServiceInstanceId() {
        return this.serviceInstanceId;
    }

    /**
     * Get the ID of the service binding.
     *
     * <p>
     * This value is set from the {@literal :binding_id} path element of the request from the platform.
     *
     * @return the service instance ID
     */
    public String getServiceBindingId() {
        return this.serviceBindingId;
    }

    /**
     * Get the value of the {@literal operation} field previously provided to the platform.
     * <p>
     * Service brokers can optionally return an {@literal operation} value to the platform in the response from an
     * async create, update, or delete request. The platform will pass this value back to the broker if provided.
     *
     * <p>
     * This value is set from the {@literal operation} request parameter of the request from the platform.
     *
     * @return the operation value
     */
    public String getOperation() {
        return this.operation;
    }

    /**
     * Create a builder that provides a fluent API for constructing a {@literal GetLastBindOperationRequestBuilder}.
     *
     * <p>
     * This builder is provided to support testing of
     * {@link ServiceInstanceBindingService} implementations.
     *
     * @return the builder
     */
    public static GetLastBindOperationRequestBuilder builder() {
        return new GetLastBindOperationRequestBuilder();
    }

    @Override
    public final boolean equals(Object o) {
        if (this == o) return true;
        if (!(o instanceof GetLastBindingOperationRequest)) return false;
        if (!super.equals(o)) return false;
        GetLastBindingOperationRequest that = (GetLastBindingOperationRequest) o;
        return Objects.equals(serviceInstanceId, that.serviceInstanceId) &&
                Objects.equals(serviceBindingId, that.serviceBindingId) &&
                Objects.equals(operation, that.operation);
    }

    @Override
    public final boolean canEqual(Object other) {
        return (other instanceof GetLastBindingOperationRequest);
    }

    @Override
    public final int hashCode() {
        return Objects.hash(super.hashCode(), serviceInstanceId, serviceBindingId, operation);
    }

    @Override
    public String toString() {
        return super.toString() +
                "GetLastBindingOperationRequest{" +
                "serviceInstanceId='" + serviceInstanceId + '\'' +
                "serviceBindingId='" + serviceBindingId + '\'' +
                ", operation='" + operation + '\'' +
                '}';
    }

    /**
     * Provides a fluent API for constructing a {@link GetLastBindingOperationRequest}.
     */
    public static class GetLastBindOperationRequestBuilder {
        private String serviceInstanceId;
        private String serviceBindingId;
        private String operation;
        private String platformInstanceId;
        private String apiInfoLocation;
        private Context originatingIdentity;

        GetLastBindOperationRequestBuilder() {
        }

        /**
         * Set the service instance ID as would be provided in the request from the platform.
         *
         * @param serviceInstanceId the service instance ID
         * @return the builder
         * @see #getServiceInstanceId()
         */
        public GetLastBindOperationRequestBuilder serviceInstanceId(String serviceInstanceId) {
            this.serviceInstanceId = serviceInstanceId;
            return this;
        }

        /**
         * Set the service binding ID as would be provided in the request from the platform.
         *
         * @param serviceBindingId the service instance ID
         * @return the builder
         * @see #getServiceInstanceId()
         */
        public GetLastBindOperationRequestBuilder serviceBindingId(String serviceBindingId) {
            this.serviceBindingId = serviceBindingId;
            return this;
        }

        /**
         * Set the operation as would be provided in the request from the platform.
         *
         * @param operation the operation
         * @return the builder
         */
        public GetLastBindOperationRequestBuilder operation(String operation) {
            this.operation = operation;
            return this;
        }

        /**
         * Set the ID of the platform instance as would be provided in the request from the platform.
         *
         * @param platformInstanceId the platform instance ID
         * @return the builder
         * @see #getPlatformInstanceId()
         */
        public GetLastBindOperationRequestBuilder platformInstanceId(String platformInstanceId) {
            this.platformInstanceId = platformInstanceId;
            return this;
        }

        /**
         * Set the location of the API info endpoint as would be provided in the request from the platform.
         *
         * @param apiInfoLocation the API info endpoint location
         * @return the builder
         * @see #getApiInfoLocation()
         */
        public GetLastBindOperationRequestBuilder apiInfoLocation(String apiInfoLocation) {
            this.apiInfoLocation = apiInfoLocation;
            return this;
        }

        /**
         * Set the identity of the user making the request as would be provided in the request from the platform.
         *
         * @param originatingIdentity the user identity
         * @return the builder
         * @see #getOriginatingIdentity()
         */
        public GetLastBindOperationRequestBuilder originatingIdentity(Context originatingIdentity) {
            this.originatingIdentity = originatingIdentity;
            return this;
        }

        /**
         * Construct a {@link GetLastBindingOperationRequest} from the provided values.
         *
         * @return the newly constructed {@literal GetLastBindingOperationRequest}
         */
        public GetLastBindingOperationRequest build() {
            return new GetLastBindingOperationRequest(serviceInstanceId, serviceBindingId,
                    operation,
                    platformInstanceId, apiInfoLocation, originatingIdentity);
        }
    }

}
